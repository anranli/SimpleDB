	...
	TransactionAbortedException {
		...
		if (gfield == Aggregator.NO_GROUPING) {
			if (this.child.getTupleDesc().getFieldType(this.afield) == Type.STRING_TYPE){
				temp = new StringAggregator(Aggregator.NO_GROUPING, null, 0, this.aggregateOp());
			}
			else if (this.child.getTupleDesc().getFieldType(this.afield) == Type.INT_TYPE){
				temp = new IntegerAggregator(Aggregator.NO_GROUPING, null, 0, this.aggregateOp());
			}
			else {
			...
			if (this.child.getTupleDesc().getFieldType(this.afield) == Type.STRING_TYPE){
				temp = new StringAggregator(0, this.child.getTupleDesc().getFieldType(this.groupField()), 1, this.aggregateOp());
			}
			else if (this.child.getTupleDesc().getFieldType(this.afield) == Type.INT_TYPE){
				temp = new IntegerAggregator(0, this.child.getTupleDesc().getFieldType(this.groupField()), 1, this.aggregateOp());
			}
			else {
				//throw an error if not one of these types
				throw new NoSuchElementException("Only String and Int Aggregators are supported.");
		...
		child.open();
		while (child.hasNext()) {
			Tuple oldTup = child.next();
			Tuple newTup = new Tuple(desc);
			if (gfield == Aggregator.NO_GROUPING) {
				newTup.setField(0, oldTup.getField(afield));
			}
			else {
				newTup.setField(0, oldTup.getField(gfield));
				newTup.setField(1, oldTup.getField(afield));
			}
			temp.mergeTupleIntoGroup(newTup);
		}
		child.close();
