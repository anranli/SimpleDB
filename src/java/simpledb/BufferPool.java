...
public class BufferPool {
	...
	HashMap<PageId, Page> bufferPoolDB;
	...
	public BufferPool(int numPages) {
		...
		bufferPoolDB = new HashMap<PageId, Page>();
	}
	...
	public Page getPage(TransactionId tid, PageId pid, Permissions perm)
		...
		if (bufferPoolDB.containsKey(pid)) {
			return bufferPoolDB.get(pid);
		} else {
			if (bufferPoolDB.size() >= totalNumberPages) {
				throw new DbException("not implemented");
			}
			bufferPoolDB.put(pid, Database.getCatalog().getDbFile(pid.getTableId()).readPage(pid));
			return bufferPoolDB.get(pid);
		}
